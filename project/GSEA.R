...
dge_heart= subset(dge_df, rownames(dge_df) %in% heart$`Gene Script`)
...
dge_mapped_df <- data.frame(
  ...
  dplyr::inner_join(dge_heart, by = c("Ensembltrans" = rownames(dge_heart)))
head(dge_mapped_df)
...
filtered_dge_mapped_df <- dge_mapped_df %>%
  ...
  dplyr::arrange(dplyr::desc(abs(dge_mapped_df[,11]))) %>%
...
t_vector <- filtered_dge_mapped_df[,11]
